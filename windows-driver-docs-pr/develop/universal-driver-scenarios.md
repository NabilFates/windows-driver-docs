...
In [`osrfx2_DCHU_extension.inx`], Contoso uses the extension to override the **OperatingParams** registry value and also add **OperatingExceptions**:
...
Fabrikam needs to register a COM component without using a co-installer. In order to accomplish this in a universal INF file, they use the [reg2inf tool](https://docs.microsoft.com/en-us/windows-hardware/drivers/devtest/reg2inf) distributed in the WDK. After building their COM server project (taken from the [In-process ATL COM server sample](https://code.msdn.microsoft.com/ATLDllCOMServer-b52a7d5d)), they take their COM .dll and provide it as an input to the reg2inf tool. The tool then generates the following INF directives that Fabrikam then includes in their base INF:
